...
contract Lottery {
    ...
    // 抽選
    function decide() public onlyOwner {
        uint randIndex = block.timestamp.mod(lotteryInfo.maxSales);
        require(randIndex < purchases.length, "当選者はいませんでした。。。");

        address payable addr = purchases[randIndex];
        addr.transfer(lotteryInfo.winningAmount);
    
        // log
        emit Decide(randIndex);
    }
